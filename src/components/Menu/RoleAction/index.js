...
console.log(props);
const {actionList=[], actions,dialog,toggleDialog,roleactionList,onClickDeleteAction} = props;
return(
<Grid container>
<Grid item md ={12} className = "icon">
<AddCircleIcon button style = {{fontSize : 40, color : "gray"}} dialog = {dialog} onClick ={()=>{toggleDialog("Create Role Action","Update")}} />
</Grid>
<Grid item md ={1} ></Grid>
<Grid item md ={10} >
<Paper>
  ...
  <TableRow>
       ...
       <TableCell>
          {roleActions.actionNames.map((roleactionname,index)=>{
            return(
              <TableRow>
                <TableCell><ol>{roleactionname}
                </ol></TableCell>
                <TableCell><Button color ="primary" >Edit</Button>
                <Button color ="primary"onClick ={() => {onClickDeleteAction(roleactionname.id)}} >Delete</Button></TableCell>
</TableRow>
);
 })}
  </TableCell>
...
const mapStateToProps=({actionList,actions,dialog,roleactionList})=>{
  return{
  actionList,actions,dialog,roleactionList
};
};
