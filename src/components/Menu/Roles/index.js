...
import {toggleDialog,onClickUpdateUserRole,onClickDeleteAction} from '../../../redux/actions';
const Roles = props => {
  console.log(props);
  ...
  return (
    <Grid container>
      <Grid item md={12} className="icon">
        <AddCircleIcon button style={{ fontSize: 40, color: "gray" }} dialog = {dialog} onClick ={() => {toggleDialog("Create Role", "Create")}}/>
      </Grid>
      <Grid item md={1}></Grid>
      <Grid item md={10}>
        <Paper>
          <Table>
            <TableHead>
              ...
              {roleList.map((roles,index)=>{
                ...
                <TableCell>
                  <Button color="primary" onClick ={()=>{toggleDialog("Update Roles","Update",roles.id)}}>Edit</Button>
